...
public:
  ...
  static Status Create(const std::string& fname,
                        WritableFile** p_file,
    ...
    if (fallocate(fd, 0, 0, init_size) != 0) {
      close(fd);
      return Status::IOError(std::string("fallocate '").append(fname)
                              .append("' failed: ").append(strerror(errno)));
    }
  ...
  static Status Reuse(const std::string& old_fname,
    ...
    if (fd < 0) {
      return Status::IOError(std::string("open '").append(old_fname)
                              .append("' failed: ").append(strerror(errno)));
    }
    // get the file size
    struct stat stat;
    if (fstat(fd, &stat) < 0) {
      close(fd);
      return Status::IOError(std::string("fstat '").append(old_fname).
                              append("' failed: ").append(strerror(errno)));
    }
    if ((size_t)stat.st_size < init_size) {
      close(fd);
      return Status::InvalidArgument(std::string("the file '").append(old_fname)
                                      .append("' to reuse is less than init_size"));
    }
    // rename it
    if (rename(old_fname.c_str(), new_fname.c_str()) < 0) {
      close(fd);
      return Status::IOError(std::string("rename '").append(old_fname)
                              .append("' to '").append(new_fname)
                              .append("' failed: ").append(strerror(errno)));
    }
    return MapFile(fd, init_size, size_addition, p_file);
  }
  ...
  Status Append(const Slice& slice) override {
    ...
    if (least_file_size > file_size) {
      ...
      if (fallocate(fd, 0, file_size, add_size) != 0) {
        return Status::IOError(std::string("expand file failed: ")
      ...
      void* new_map_base = mmap(0, new_map_length, PROT_WRITE,
                                MMAP_FLAGS, fd, offset_aligned);
      if (new_map_base == MAP_FAILED) {
        return Status::IOError(std::string("mmap new range failed: ")
                                .append(strerror(errno)));
      }
...
private:
  ...
  static Status MapFile(int fd, size_t init_size, size_t size_addition,
    ...
    if (p_length == MAP_FAILED) {
      close(fd);
      return Status::IOError(std::string("mmap first size_t failed: ")
                              .append(strerror(errno)));
    }
    // whole file
    ...
    if (base == MAP_FAILED) {
      close(fd);
      munmap(p_length, sizeof(size_t));
      return Status::IOError(std::string("mmap whole file failed: ")
                              .append(strerror(errno)));
    }
    *p_file = new DCPMMWritableFile(fd, (size_t*)p_length, base,
                                              init_size, size_addition);
    return Status::OK();
  }
  ...
  DCPMMWritableFile(int _fd, size_t* _p_length, void* base,
    size_t init_size, size_t _size_addition) :
    data_length(0), file_size(init_size), size_addition(_size_addition),
    fd(_fd), p_length(_p_length), map_base(base), map_length(init_size),
...
private:
  DCPMMSequentialFile(void* base, size_t _file_size) :
...
static bool EndsWith(const std::string& str, const std::string& suffix) {
  ...
  return memcmp(str.c_str() + str_len - suffix_len,
                suffix.c_str(), suffix_len) == 0;
}
...
Status DCPMMEnv::DeleteFile(const std::string& fname) {
  ...
  if (rename(fname.c_str(), tmp_fname.c_str()) < 0) {
    return Status::IOError(std::string("rename '").append(fname)
                            .append("' to '").append(tmp_fname)
                            .append("' failed: ").append(strerror(errno)));
  }
  lock_recycle_logs.lock();
